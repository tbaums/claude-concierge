...
mkdir -p "$CFG"
cp "$REPO_DIR"/config/{tmux.conf,start.sh,clip.sh,logsink.sh,session-menu.sh,status-model.sh,model-label.sh} "$CFG/"
cp "$REPO_DIR/VERSION" "$CFG/VERSION"
chmod +x "$CFG"/{start.sh,clip.sh,logsink.sh,session-menu.sh,status-model.sh}
